...
import { models, type ModelInstanceOptions } from "feathers-pinia";

export class User extends BaseModel {
  // Definining properties here can trigger a ts error :
  // "Property 'id' has no initializer and is not definitely assigned in the constructor."
  // To remove it, add a ! after variable name : id!: Id
  // For me, I prefer to remove this ts option by adding this to tsconfig.json
  ...
  // fixes interface properties (above) are undefined (i.e. id: undefined) when fetching from server
  // https://feathers-pinia.pages.dev/guide/model-classes.html#default-class-properties
  constructor(data: User, options: ModelInstanceOptions = {}) {
    // You must call `super` very first to instantiate the BaseModel
    super(data, options);

    const { store, instanceDefaults } = this.constructor as typeof BaseModel;
